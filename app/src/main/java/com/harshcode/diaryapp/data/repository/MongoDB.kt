package com.harshcode.diaryapp.data.repository
...
object MongoDB : MongoRepository {
    ...
    override fun configureTheRealm() {
        if (user != null) {
            val config = SyncConfiguration.Builder(user = user, setOf(Diary::class))
    ...
    override fun getFilteredDiaries(zonedDateTime: ZonedDateTime): Flow<Diaries> {
        return if (user != null) {
            try {
            realm.query<Diary>("ownerId == $0 AND date < $1 AND date > $2",
                user.id,
